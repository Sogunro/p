...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${EMBEDDING_SERVICE_URL}/agent/analyze-session`, {
      method: 'POST',
      headers: {
    ...
    console.error('Session Analyzer error:', error)
    return NextResponse.json({ error: 'Failed to run Session Analyzer' }, { status: 500 })
  }
}
